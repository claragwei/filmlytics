...
# Navigation key FIX — prevents Streamlit crashes
...
def load_artifacts():
    ...
    for key, filename in files.items():
        ...
        if filename.endswith(".csv"):
            ...
            col = (
                "pred_audience_score"
                if "pred_audience_score" in df.columns else
                "predicted_audience_score"
                if "predicted_audience_score" in df.columns else
                "ensemble_pred"
            )
...
def get_genre_neighbors(db, genre, limit=15):
    return list(db.movies.find({"production.genres": genre}).limit(limit))
...
def build_graph(db, movie, max_neighbors=15):
    ...
    neighbors = get_genre_neighbors(db, genres[0], limit=max_neighbors)
...
# -----------------------------------------------------------------------------
# HOME PAGE (INTRO ONLY — NO CARDS, NO GRAPH MOVE)
# -----------------------------------------------------------------------------
...
def page_home(db):
    ...
    # simple nav buttons
    cols = st.columns(4)
    for label, col in zip(["Home", "Movie Search", "Compare Movies", "Analytics Dashboard"], cols):
        if col.button(label):
    ...
    # INTRO TEXT ONLY (working version)
...
Studios depend on audience reception for marketing, streaming placement, and long-term success.
Our system explores which film characteristics and representation patterns influence audience response.
""")

    st.header("Our Approach")
    st.write("""
- Collect data from TMDB, Rotten Tomatoes, and YouTube  
- Clean & merge data into MongoDB  
- Engineer 150+ features including metadata, engagement, and diversity  
- Train GNN, KGCN, and XGBoost  
- Combine outputs with a stacking meta-learner for optimal performance  
""")
...
- KGCN RMSE: **0.171**  
- GNN RMSE: **0.195**  
- Stacking Ensemble RMSE: **0.1085**  
""")


# -----------------------------------------------------------------------------
# MOVIE SEARCH PAGE (UNCHANGED)
# -----------------------------------------------------------------------------
...
def page_movie_search(db, artifacts):
    ...
    st.metric("Predicted Audience Score", f"{pred*100:.1f}%" if pd.notna(pred) else "N/A")
...
def page_analytics(db):
    ...
    movies = db.movies.find({}, {"tmdb_metrics.vote_average": 1})
...
Learns similarity patterns between movies.

### Knowledge Graph Convolutional Network (KGCN)
Learns entity-level relationships.

### XGBoost
Over 150 engineered features including metadata, engagement & representation.
""")
...
def page_graph(db):
    ...
    titles = all_titles(db)
    choice = st.selectbox("Select a movie:", ["-- Select --"] + titles)
...
Data from TMDB, Rotten Tomatoes, YouTube API.

Thanks to STA 160 instructors for support.
""")
